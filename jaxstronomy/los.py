...
def mass_function_exact(
    ...
    sigma = jax.vmap(cosmology_utils.sigma_tophat, in_axes=[None, 0, None])(
    ...
    derivative_sigma = cosmology_utils.derivative_log_sigma_log_r(
        cosmology_params, lagrangian_r, z)
    # Matter density is returned in units of M_sun * h ^ 2 / Mpc ^ 3
    rho_matter = cosmology_utils.rho_matter(cosmology_params, z) * h ** 2
    rho_matter *= 1e-9  # Convert to / kpc ^ 3
...
def _mass_function_power_law_numerical(
    ...
    log_mass_function = jnp.log(
        mass_function_exact(cosmology_params, masses, z))

    # Use the MLE estimate for the slope assuming Gaussian scatter on the log
    # quantity.
    slope_estimate = 1 / num_masses * jnp.sum(log_mass_function) * jnp.sum(
        log_masses)
...
def cone_angle_to_radius(main_deflector_params: Mapping[str, float],
    ...
    # What fraction of the contraction to include. 1.0 will have the cone
    # contract to a point at the source redshift. 0.0 will have the cone not
    # contract at all after passing the main deflector.
...
def draw_redshifts(main_deflector_params: Mapping[str, float],
    ...
    Args:
        ...
        cosmology_params: Cosmological parameters that define the universe's
            expansion.
        z_min: Minimum redshift of interval. Lower bound of lowest redshift
            interval considred [z_min, z_min + dz].
        z_max: Maximum redshift of interval. Upper bound of highest redshift
            interval considred [z_max - dz, z_max].
        rng: jax PRNG key.
    ...
    z_draws = jnp.round(z_draws / dz) * dz

    # More accurate to give the halos a redshift between the two slices.
    z_draws += dz / 2
...
def draw_los(main_deflector_params: Mapping[str, float],
    ...
    los_before_z = jnp.sort(los_before_z)
    ...
    z_source = source_params['z_source']
    los_after_z = draw_redshifts(main_deflector_params, source_params,
        los_params, cosmology_params, z_lens, z_source, rng_after_z,
        num_z_bins, los_pad_length)
    los_after_z = jnp.sort(los_after_z)
    los_after_masses = draw_masses(los_params, cosmology_params, los_after_z,
        rng_after_mass)
    los_after_cart_pos = draw_positions(main_deflector_params, source_params,
        los_params, cosmology_params, los_after_z, rng_after_pos)
    los_after_tuple = convert_to_lensing(main_deflector_params, source_params,
        los_params, cosmology_params, los_after_masses, los_after_z,
        los_after_cart_pos, rng_after_convert)

    # Set the padding to the minimum redshift
    is_pad = los_before_z == -1.0
    los_before_z = los_before_z * ~is_pad + is_pad * jnp.min(los_before_z)
    is_pad = los_after_z == -1.0
    los_after_z = los_after_z * ~is_pad + is_pad * jnp.min(los_after_z)
