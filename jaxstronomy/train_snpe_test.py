...
class TrainSNPETests(chex.TestCase, parameterized.TestCase):
    ...
    def test_proposal_distribution_update(self):
        ...
        log_var_bound = jnp.zeros(5)

        mu_prop, prec_prop = train_snpe.proposal_distribution_update(
            current_posterior, mean_norm, std_norm, log_var_bound, input_config)
        ...
        for i in range(len(input_config['truth_parameters'][0])):
            ...
            self.assertAlmostEqual(mu_prop[0][i] * std_norm[i] + mean_norm[i],
                                   object_distribution[4])
            self.assertAlmostEqual(1/jnp.sqrt(prec_prop[0][i,i]) * std_norm[i],
                                   object_distribution[5])

        # Check that the values math the posterior with the bounds applied.
        np.testing.assert_array_almost_equal(current_posterior[0][:5],
                                             mu_prop[0])
        np.testing.assert_array_almost_equal(
            jnp.diag(jnp.array([1.0, 1.0, 1/0.25, 1.0, 1.0])), prec_prop[0])
