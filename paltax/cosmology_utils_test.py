...
def _prepare_cosmology_params(
    cosmology_params_init, z_lookup_max, dz, r_min=1e-4, r_max=1e3, n_r_bins=2
):
...
def _prepare__sigma_k_integrand_expected(radius):
    if radius == 1e-2:
        return jnp.array([
            3.127437136035467e-06, 1.820751207237299e-05,
            5.2541055609899326e-05, 0.00010306278738724287,
            0.00010775093864571042
        ])
    if radius == 1e-1:
        return jnp.array([
            3.127323720477859e-06, 1.8185657328033008e-05,
            5.04866815266023e-05, 2.3545351279405076e-05,
            1.5387621097147294e-08
        ])
    if radius == 1e0:
        return jnp.array([
            3.116000043964669e-06, 1.611047820558932e-05,
            1.534716000730611e-10, 1.2524276267847e-09,
            1.6400363103856087e-12
        ])
...
def _prepare_reduced_to_physical_expected(z_lens, z_source):
    ...
    if z_lens == 0.5 and z_source == 1.0:
        return jnp.array([-1.1832292, 8.830428, -7.035888])
    if z_lens == 0.5 and z_source == 2.0:
...
def _preapre_comoving_to_angle_expected(z_lens):
    if z_lens == 0.1:
        return jnp.array([
            [-3.1144106, -1.2556583, 0.8742881],
            [1.524639, -4.0299582, 3.8287995]]
        )
    elif z_lens == 0.5:
        return jnp.array([
            [-0.6921371, -0.2790537, 0.19429912],
            [0.33883113, -0.8956057, 0.85090077]]
        )
    elif z_lens == 1.0:
        return jnp.array([
            [-0.39674094, -0.15995677, 0.11137448],
...
class CosmologyUtilsTest(chex.TestCase, parameterized.TestCase):
    ...
    def test__e_z_rad_to_dark(self):
        ...
        np.testing.assert_allclose(
            e_z_rad_to_dark(dict(COSMOLOGY_PARAMS_INIT), z_values),
            expected)

    @chex.all_variants
    @parameterized.named_parameters([
        (f'_z_min_{z_min}_z_max_{z_max}', z_min, z_max)
        for z_min, z_max in zip([0.0, 0.0, 0.0, 0.5],
                    [0.0, 1.283, 20.0, 1.283])
    ])
    ...
    @parameterized.named_parameters([
        (f'_r_{r}', r, expected)
        for r, expected in zip([1e-2, 1e-1, 1e0],
                   [0.004892, 0.002956, 0.001396])
    ])
    def test__sigma_numerical(self, r, expected):
        sigma_numerical = self.variant(cosmology_utils._sigma_numerical)

        np.testing.assert_allclose(
            sigma_numerical(dict(COSMOLOGY_PARAMS_INIT), r), expected,
            rtol=1e-3)
    ...
    def test___growth_factor_exact_unormalized(self, z, expected):
        ...
        self.assertAlmostEqual(
            growth_factor(dict(COSMOLOGY_PARAMS_INIT), z), expected,
            places=3)

    @chex.all_variants
    @parameterized.named_parameters([
        (f'_z_{r}', r, expected) for r, expected in zip(
            [1e-2, 1e-1, 1e2], [71.204, 25.504, 0.002])
    ])
    def test___correlation_function_exact(self, radius, expected):
        # Test some set correlation function values.
        correlation_function_exact = self.variant(
            cosmology_utils._correlation_function_exact
        )
        self.assertAlmostEqual(
            correlation_function_exact(dict(COSMOLOGY_PARAMS_INIT), radius),
            expected, places=2
        )

    @parameterized.named_parameters([
        (f'_z_l_max_{z_l_max}_dz_{dz}_r_max_{r_max}', z_l_max, dz, r_max)
        for z_l_max, dz, r_max in zip([1.01, 2.02], [0.01, 0.02],
                      [1e-2, 1e1])
    ])
    def test_add_lookup_tables_to_cosmology_params(self, z_l_max, dz, r_max):
        ...
        growth_factor = jax.jit(
            cosmology_utils._growth_factor_exact_unormalized
        )
        correlation_function = jax.jit(
            cosmology_utils._correlation_function_exact
        )
        ...
        self.assertAlmostEqual(
            ...
            sigma_numerical(cosmology_params, r_max) *
            sigma_norm(cosmology_params),
            places=3)
        ...
        # Check that the correlation lookup table is correctly written.
        i = 1
        self.assertAlmostEqual(
            cosmology_params['correlation_lookup_table'][i],
            correlation_function(cosmology_params, r_max),
            places=3)
    ...
    # Test that inerpolation returns approximately the correct result even
    # when the sampling doesn't fall perfectly on the grid.
    ...
    @chex.all_variants
    @parameterized.named_parameters([(f'_z_{z}', z)
                    for z in [0.0, 1.283, 20.0]])
    def test_angular_diameter_distance(self, z):
        cosmology_params = _prepare_cosmology_params(
            COSMOLOGY_PARAMS_INIT, z, z
        )
    ...
    def test_angular_diameter_distance_between(self, z_min, z_max):
        cosmology_params = _prepare_cosmology_params(
            COSMOLOGY_PARAMS_INIT, z_max, z_max
        )
        expected = _prepare_expected_comoving_distance(z_min, z_max)
        expected /= (1 + z_max)
    ...
    @chex.all_variants
    @parameterized.named_parameters(
        [(f'_z_{z}', z) for z in [0.0, 1.283, 20.0]]
    )
    ...
    def test_comoving_to_angle(self, z_lens):
        x, y = _prepare_x_y()
        cosmology_params = _prepare_cosmology_params(
            COSMOLOGY_PARAMS_LENSTRONOMY, z_lens, z_lens
        )
    ...
    def test_reduced_to_physical(self, z_lens, z_source):
        ...
        # Division and multiplication by two quantities with errors in 1e-6, so
        # need to increase rtol for this test significantly.
        np.testing.assert_allclose(
    ...
    def test_rho_matter(self, omega_m_zero, z, expected):
        ...
        self.assertAlmostEqual(
            rho_matter({'omega_m_zero': omega_m_zero}, z) / 1e9, expected,
            places=3
        )
    ...
    def test_collapse_overdensity(self, z, expected):
        ...
        collapse_overdensity = self.variant(
            cosmology_utils.collapse_overdensity
        )
    ...
    def test_lagrangian_radius(self, m_log10, expected):
        cosmology_params = _prepare_cosmology_params(
            COSMOLOGY_PARAMS_INIT, 0.1, 0.1
        )
        lagrangian_radius = self.variant(cosmology_utils.lagrangian_radius)

        self.assertAlmostEqual(
            lagrangian_radius(cosmology_params, 10**m_log10), expected,
            places=3
        )
    ...
    @parameterized.named_parameters([
        (f'_z_{z}', z, expected) for z, expected in zip(
            [0.1, 0.2, 0.3], [0.9489, 0.9001, 0.8541]
        )
    ])
    def test_growth_factor(self, z, expected):
        cosmology_params = _prepare_cosmology_params(
            COSMOLOGY_PARAMS_INIT, 1.0, 0.1
        )
        growth_factor = self.variant(cosmology_utils.growth_factor)
        self.assertAlmostEqual(
            growth_factor(cosmology_params, z), expected, places=3
        )
    ...
    def test_peak_height(self, m, z, expected):
        # Figure out what lagrangian radius we need for the peak height
        # calculation.
        cosmology_params = _prepare_cosmology_params(COSMOLOGY_PARAMS_INIT, z,
            z)
    ...
    def test_calculate_sigma_crit(self, z, z_source, expected):
    ...
    @chex.all_variants
    @parameterized.named_parameters([
    (f'_r_{r}_z_{z}', r, z, expected) for
        r, z, expected in zip([1e-2, 1e-1, 1e1], [0.2, 0.4, 0.6],
        [57.6886, 16.7685, 0.1906])
    ])
    def test_correlation_function(self, radius, z_lens, expected):
        # Test a few reference values of the critical surface overdensity.
        cosmology_params = _prepare_cosmology_params(COSMOLOGY_PARAMS_INIT,
            z_lens, 0.1, 1e-4, radius, n_r_bins=2)
        correlation_function = self.variant(
            cosmology_utils.correlation_function
        )
        self.assertAlmostEqual(
            correlation_function(cosmology_params, radius, z_lens),
            expected, places=2
        )
